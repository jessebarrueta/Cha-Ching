...
type ChildInvite = {
  ...
  expires_at: string;
  accepted_child_user_id: string | null;
};
...
Deno.serve(async (request) => {
  ...
  const { data: inviteData, error: inviteError } = await serviceClient
    .from("child_invites")
    .select("id,family_id,child_profile_id,child_name,status,expires_at,accepted_child_user_id")
  ...
  const invite = inviteData as ChildInvite;

  if (invite.status === "accepted") {
    if (invite.accepted_child_user_id === userData.user.id) {
  ...
  const { data: linkedProfile, error: profileError } = await serviceClient
    .from("child_profiles")
    .update({ linked_user_id: userData.user.id })
    .eq("id", invite.child_profile_id)
    .or(`linked_user_id.is.null,linked_user_id.eq.${userData.user.id}`)
    .select("id")
    .maybeSingle();

  if (profileError) {
    console.error(profileError);
    return json({ error: "profile_link_failed" }, 500);
  }

  if (!linkedProfile) {
    return json({ error: "child_profile_already_linked" }, 409);
  }
  ...
  const { data: acceptedInviteData, error: updateError } = await serviceClient
    ...
    .update({
      ...
      accepted_child_user_id: userData.user.id,
    })
    .eq("id", invite.id)
    .select("id,family_id,child_profile_id,child_name,status,expires_at,accepted_child_user_id")
    .single();
  ...
  return acceptedResponse(acceptedInviteData as ChildInvite, userData.user.id);
});
...
function acceptedResponse(invite: ChildInvite, userId: string): Response {
  return json({
    family_id: invite.family_id,
    child_profile_id: invite.child_profile_id,
    child_name: invite.child_name,
    accepted_child_user_id: userId,
  });
}
